...
1. In Netlify, choose **Add new project** → **Import an existing project**, connect GitHub, and select your fork.
2. On the import screen, verify the build settings instead of accepting an unrelated framework preset:
   ...
   These values are also defined in `netlify.toml`. However, a build command saved in the Netlify UI takes precedence over the file. If this project was previously configured for another app, open **Project configuration** → **Build & deploy** → **Continuous deployment** → **Build settings**, select **Configure**, and replace the UI build command with `bash scripts/build-netlify.sh` (or clear the UI override so `netlify.toml` is used). Do not use `expo export -p web`, and do not install Expo in this PHP repository.

3. Create a GitHub personal access token. For a card that only uses public contribution data, a [classic token](https://github.com/settings/tokens/new?description=GitHub%20Readme%20Streak%20Stats) with no scopes selected is sufficient. Copy the generated token immediately; GitHub only shows it once. Do not put the token in this repository or in `netlify.toml`.
4. Open your project in Netlify and go to **Project configuration** → **Environment variables**. Select **Add a variable** → **Add a single variable** for each entry below. Set the deploy context to **Production** and use the specified scope:

   | Key              | Example value                                 | Scope      | Required | What to enter                                                                                                               |
   | ---------------- | --------------------------------------------- | ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------- |
   | `TOKEN`          | `github_pat_...` or `ghp_...`                 | **Builds** | Yes      | Paste the actual token generated by GitHub—not the example text. Mark it as containing a secret value if Netlify offers it. |
   | `STREAK_USER`    | `octocat`                                     | **Builds** | Yes      | Your GitHub login, without `@` and without a profile URL.                                                                   |
   | `STREAK_OPTIONS` | `theme=dark&hide_border=true&border_radius=5` | **Builds** | No       | Any supported card options as one query string, with no leading `?`. Leave this variable out for the default card.          |

   For example, if your profile is `https://github.com/janedoe`, use `STREAK_USER=janedoe`. Do not add `user=janedoe` to `STREAK_OPTIONS`; the build script always takes the username from `STREAK_USER`.

5. Select **Save**, then go to **Deploys** and choose **Trigger deploy** → **Deploy site** (or retry the failed initial deploy). Environment-variable changes only take effect in a new deploy. Open `https://YOUR-SITE.netlify.app/streak.svg` to verify the card, then add it to your GitHub profile README:

   ```md
   ![GitHub Streak](https://YOUR-SITE.netlify.app/streak.svg)
   ```

6. To refresh the card automatically, open **Project configuration** → **Build & deploy** → **Continuous deployment** → **Build hooks**, select **Add build hook**, name it `Daily streak refresh`, and select your production branch. Netlify will display a private URL similar to `https://api.netlify.com/build_hooks/abc123...`. Return to **Project configuration** → **Environment variables**, add the following variable, and trigger one more deploy:

   | Key              | Example value                                   | Scope         | Deploy context | Required |
   | ---------------- | ----------------------------------------------- | ------------- | -------------- | -------- |
   | `BUILD_HOOK_URL` | `https://api.netlify.com/build_hooks/abc123...` | **Functions** | Production     | No       |

   Paste your complete build-hook URL as the value; do not use the example URL. The included `rebuild` function posts to it daily at 00:00 UTC. Treat this URL like a secret because anyone who has it can trigger a deploy. If you omit `BUILD_HOOK_URL`, refresh the card by triggering a deploy manually.
...
Fix the project settings rather than adding `expo` or `expo-cli`:

1. Open **Project configuration** → **Build & deploy** → **Continuous deployment** → **Build settings**.
2. Select **Configure** or **Edit settings**.
3. Set **Build command** to `bash scripts/build-netlify.sh`.
4. Set **Publish directory** to `dist` and leave **Base directory** blank.
5. Save, then open **Deploys** → **Trigger deploy** → **Clear cache and deploy site**.
